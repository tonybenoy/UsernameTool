...
def response(host,username,verbose):
    if host[-1] != "/":
        host = host + "/"
    if host[0:4] != "http":
        host = "https://" + host
...
if args.username and args.host:
    ...
    print(response(args.host + args.username , args.verbose))

if (args.username and args.facebook) or (args.username and args.all):    
    print(response("https://facebook.com/" + args.username, args.verbose))

if (args.username and args.instagram) or (args.username and args.all):
    print(response("https://instagram.com/" + args.username, args.verbose))

if (args.username and args.github) or (args.username and args.all):
    print(response("https://github.com/" + args.username, args.verbose))

if (args.username and args.soundcloud) or (args.username and args.all) :
    print(response("https://soundcloud.com/" + args.username, args.verbose))
    ...
    with open(args.hostcsv) as csv_file:
        ...
        for row in csv_reader:
            for host in row:
                ...
                if args.postfix:
                    args.username = args.username + args.postfix 
                print(response(host , args.username , args.verbose))
...
if args.usercsv and (args.host or args.instagram or args.facebook or args.soundcloud or args.github):
    with open(args.usercsv) as csv_file:
        ...
        for row in csv_reader:
            for username in row:
                if args.github:
                    print(response("https://github.com" , username , args.verbose))
                if args.facebook:
                    print(response("https://facebook.com" , username , args.verbose))
                if args.instagram:
                    print(response("https://instagram.com" , username , args.verbose))
                if args.soundcloud:
                    print(response("https://soundcloud.com" , username , args.verbose))
                if args.prefix:
                    username = args.prefix + username
                if args.postfix:
                    username = username + args.postfix 
                if args.host:
                    print(response(args.host, username, args.verbose))
        ...
        for userrow in usercsv_reader:
            for username in userrow:
                with open(args.hostcsv) as csv_file:
                    ...
                    for row in csv_reader:
                        for host in row:
                            print(response(host, username, args.verbose))
